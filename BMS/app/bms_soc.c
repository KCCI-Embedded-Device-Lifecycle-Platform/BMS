 ...
 *
 * @details 왜 전압 기반인가?
 *      정확도만 보면 쿨롱 카운팅(전류 적분)이 낫지만,
 *      적분 오차가 시간에 따라 누적되고 초기 SOC 를 모른다는 문제가 있다.
 *      실제 BMS 는 "쿨롱 카운팅 + 무부하 시 OCV 로 재보정" 하이브리드를 쓴다.
 *      1단계에서는 구조만 잡아 두고 OCV 기반으로 구현한다.
 *      (2단계에서 전류 적분을 추가할 자리를 함수로 남겨 둠)
 *
 * @note    전압 기반의 한계
 *      충/방전 중에는 내부저항 강하(I x R) 때문에 단자 전압이
 *      실제 OCV 와 다르다. 그래서 SOC 가 출렁인다.
 *      -> IIR 필터로 평활하고, 1초 주기로만 갱신한다.
 *         SOC_new = SOC_old + (SOC_raw - SOC_old) / 8
 */
#include "bms_soc.h"
#include "dbg.h"
